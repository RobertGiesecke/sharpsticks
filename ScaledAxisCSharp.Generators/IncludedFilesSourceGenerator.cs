...
public sealed class IncludedFilesSourceGenerator : IIncrementalGenerator
{
	...
	public void Initialize(IncrementalGeneratorInitializationContext context)
	{
		...
		var assemblyTarget = context.CompilationProvider.Select(static (compilation, _) =>
		{
			...
			return new AssemblyDeviceInfoTarget(
				...
				GetDeviceRenames(attributes),
				GetAxisRenames(attributes),
				GetButtonRenames(attributes));
		});
	...
	private static void GenerateAssemblyDeviceInfos(SourceProductionContext context, AssemblyDeviceInfoTarget target)
	{
		...
		usings.AppendLine("global using static Devices;");
		if (HasLevel(target.Levels, GenerateDeviceInfosLevels.TypedDevices))
		{
			usings.AppendLine("global using static Devices.Typed;");
		}

		context.AddSource("Devices.GlobalUsings.g.cs", SourceText.From(usings.ToString(), Encoding.UTF8));
	}
		...
		ImmutableArray<VJoyDeviceSnapshot> outputDevices)
	{
		...
		AppendDeviceMembers(builder, levels, deviceRenames, axisRenames, buttonRenames, directInputDevices, outputDevices,
		...
		int indentLevel)
	{
		...
		if (HasLevel(levels, GenerateDeviceInfosLevels.TypedDevices))
		{
			for (var index = 0; index < directInputDevices.Length; index++)
			{
				...
				var axisPropertyNames = BuildAxisPropertyNames(directInputDevices[index].ProductName, directInputNames[index], axisRenames);
				var buttonPropertyNames = BuildButtonPropertyNames(directInputDevices[index].ProductName, directInputNames[index], buttonRenames);
				AppendSeparator(builder, ref wroteMember);
			...
			foreach (var outputDevice in outputDevices.OrderBy(static d => d.DeviceId))
			{
				...
				var outputButtonNames = BuildButtonPropertyNames(deviceName, vjoyBaseName, buttonRenames, vjoyIdentifier);
				AppendSeparator(builder, ref wroteMember);
				AppendTypedOutputDeviceClass(builder, vjoyIdentifier, vjoyBaseName, outputDevice.Axes, outputAxisNames, outputDevice.ButtonCount, outputButtonNames, indent, memberIndent);
			}
		...
		string memberIndent)
	{
		...
		if (device.ButtonCount > 0)
		{
			...
			for (var btn = 1; btn <= device.ButtonCount; btn++)
			{
				...
				{
					builder.Append(innerMemberIndent).Append("/// <summary>Button ").Append(btn).AppendLine("</summary>");
				}
		...
		string memberIndent)
	{
		...
		builder.Append(innerIndent).Append("public const string DeviceName = DeviceNames.").Append(vjoyOriginalName).AppendLine(";");
		builder.Append(innerIndent).Append("public const uint DeviceId = OutputDeviceIds.").Append(vjoyOriginalName).AppendLine(";");
		...
		if (axes.Length > 0)
		{
			...
			builder.Append(innerMemberIndent)
				.AppendLine("\t_ => throw new ArgumentOutOfRangeException(nameof(axis), $\"Axis {axis} is not defined for {DeviceName}.\")");
			builder.Append(innerMemberIndent).AppendLine("};");
		}
		...
		if (buttonCount > 0)
		{
			...
			builder.Append(innerMemberIndent).AppendLine("{");
			builder.Append(innerMemberIndent).Append("\tAll = [");
			builder.Append(string.Join(", ", Enumerable.Range(1, (int)buttonCount).Select(b => buttonPropertyNames.GetValueOrDefault(b, $"Btn{b}"))));
			builder.AppendLine("];");
			builder.Append(innerMemberIndent).AppendLine("}");
			builder.Append(innerIndent).AppendLine("}");
		}
		...
		string? extraIdentifier = null)
	{
		...
		foreach (var rename in axisRenames)
		{
			if (rename.DeviceName == deviceProductName || rename.DeviceName == deviceBaseIdentifier
			    || (extraIdentifier != null && rename.DeviceName == extraIdentifier))
			{
	...
	private static GenerateDeviceInfosLevels GetDeviceInfoLevels(IEnumerable<AttributeData> attributes)
	{
		...
		{
			return (GenerateDeviceInfosLevels)GetTypedConstantInt32(attribute.ConstructorArguments[0], (int)GenerateDeviceInfosLevelsDefault);
		}
		...
		foreach (var namedArgument in attribute.NamedArguments)
		{
			...
			{
				return (GenerateDeviceInfosLevels)GetTypedConstantInt32(namedArgument.Value, (int)GenerateDeviceInfosLevelsDefault);
			}
		}
	...
	private readonly struct DeviceInfoTarget
	{
			...
			ImmutableArray<ButtonRename> buttonRenames)
		{
			Type = type;
			Levels = levels;
			DeviceRenames = deviceRenames;
			AxisRenames = axisRenames;
			ButtonRenames = buttonRenames;
		}

		public INamedTypeSymbol Type { get; }
		public GenerateDeviceInfosLevels Levels { get; }
		public ImmutableArray<DeviceRename> DeviceRenames { get; }
		public ImmutableArray<AxisRename> AxisRenames { get; }
		public ImmutableArray<ButtonRename> ButtonRenames { get; }
	}
		...
		ImmutableArray<ButtonRename> ButtonRenames)
	{
		...
		public bool Equals(AssemblyDeviceInfoTarget other) =>
			Levels == other.Levels
			&& SequenceEqual(DeviceRenames, other.DeviceRenames)
			&& SequenceEqual(AxisRenames, other.AxisRenames)
			&& SequenceEqual(ButtonRenames, other.ButtonRenames);

		public override int GetHashCode()
		{
			var hash = (int)Levels;
			hash = CombineHash(hash, DeviceRenames);
			hash = CombineHash(hash, AxisRenames);
			hash = CombineHash(hash, ButtonRenames);
			return hash;
		}
		...
		private static int CombineHash<T>(int seed, ImmutableArray<T> array)
		{
			if (array.IsDefault)
			{
				return seed;
