...
public sealed class IncludedFilesSourceGenerator : IIncrementalGenerator
{
		...
		int indentLevel)
	{
		...
		if (HasLevel(levels, GenerateDeviceInfosLevels.TypedDevices))
		{
			...
			foreach (var outputDevice in outputDevices.OrderBy(static d => d.DeviceId))
			{
				...
				AppendSeparator(builder, ref wroteMember);
				AppendTypedOutputDeviceClass(builder, vjoyIdentifier, vjoyBaseName, deviceName, outputDevice.Axes, outputAxisNames, outputDevice.ButtonCount, outputButtonNames, indent, memberIndent);
			}
	...
	private static ImmutableArray<ButtonRename> GetButtonRenames(GeneratorAttributeSyntaxContext syntaxContext)
	{
		...
		foreach (var attr in syntaxContext.TargetSymbol.GetAttributes())
		{
	...
	{
		if (expr is LiteralExpressionSyntax literal && literal.Token.Value is int i)
			return i;
		return -1;
	}
		...
		ImmutableArray<VJoyDeviceSnapshot> outputDevices)
	{
		...
		for (var index = 0; index < devices.Length; index++)
		{
			...
			{
				if (!groupCounters.TryGetValue(name, out var counter))
					counter = 0;
