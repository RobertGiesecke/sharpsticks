...
public sealed class DevicesGenerator : IIncrementalGenerator
{
	...
	public void Initialize(IncrementalGeneratorInitializationContext context)
	{
		...
		// Editor-experience bridge. VS Code / Zed load file-based programs with a design-time
		// build that runs generators (so device infos resolve) but applies neither the
		// package's buildTransitive props <Using> items NOR GenerateGlobalUsings — so the
		// package global usings (namespaces, ConsoleExtensions, Macros, PlatformDefault*
		// aliases) never reach IntelliSense. The generator's OWN output IS applied, so we
		// re-derive those usings here from the referenced SharpSticks assemblies + the host
		// OS. Gated on the absence of our usings from the compilation: in a real build
		// GenerateGlobalUsings already emitted them (props are honoured there), and emitting
		// again would duplicate every using — a fatal CS1537 for the alias forms.
		// Only opt in when the consumer referenced the SharpSticks.Editor bundle — that is the
		// package whose purpose is the editor experience. Detected by assembly reference (not a
		// props property): the language server applies lib references but not buildTransitive
		// props, so a property set in Editor's props would never reach us in the editor. A plain
		// SharpSticks.Console consumer does not reference SharpSticks.Editor and is left alone.
		var packageGlobalUsings = context.CompilationProvider.Select(static (compilation, _) =>
			ReferencesEditorBundle(compilation) && !AlreadyHasPackageGlobalUsings(compilation)
				? ComputePackageGlobalUsings(compilation)
				: null);
		...
		context.RegisterSourceOutput(
			...
			static (sourceProductionContext, source) =>
			{
				...
				GeneratorLog.Log("AddSource: SharpSticks.PackageGlobalUsings.g.cs");
				sourceProductionContext.AddSource(
					"SharpSticks.PackageGlobalUsings.g.cs", SourceText.From(source, Encoding.UTF8));
			});
	}

	// The SharpSticks.Editor bundle is the opt-in for the editor global-usings bridge. Detect
	// it by assembly reference — the one signal the language server honours for file-based
	// programs (it applies lib references but not buildTransitive props).
	private static bool ReferencesEditorBundle(Compilation compilation) =>
		compilation.SourceModule.ReferencedAssemblySymbols.Any(static a =>
			a.Name.Equals("SharpSticks.Editor", StringComparison.Ordinal));

	// True when the package global usings are already in the compilation (a real build, where
	// GenerateGlobalUsings honoured the props). An editor design-time build emits at most a
	// System.*-only GlobalUsings, so the sentinel is absent and we proceed to emit.
	private static bool AlreadyHasPackageGlobalUsings(Compilation compilation) =>
		compilation.SyntaxTrees.Any(static tree =>
			tree.FilePath.EndsWith(".GlobalUsings.g.cs", StringComparison.OrdinalIgnoreCase)
			&& tree.ToString().Contains("global using SharpSticks.InputAbstractions;"));

	private static string? ComputePackageGlobalUsings(Compilation compilation)
	{
		...
		// Namespace imports for every referenced SharpSticks.* assembly (assembly name ==
		// root namespace in this product). Only referenced assemblies are emitted, so the
		// directive never dangles.
		foreach (var name in compilation.SourceModule.ReferencedAssemblySymbols
			         .Select(static a => a.Name)
			         .Where(static n => n.StartsWith("SharpSticks.", StringComparison.Ordinal)
			                            && !n.Equals("SharpSticks.Generators", StringComparison.Ordinal))
			         .Distinct()
			         .OrderBy(static n => n, StringComparer.Ordinal))
		{
			builder.Append("global using ").Append(name).AppendLine(";");
			wrote = true;
		}

		if (HasType("SharpSticks.InputAbstractions.Macros"))
		{
			builder.AppendLine("global using static SharpSticks.InputAbstractions.Macros;");
			wrote = true;
		}

		// Resolve the platform device/factory types for the host OS, mirroring the props'
		// OS Choose. Only emit aliases whose target type is actually referenced.
		var isLinux = RuntimeInformation.IsOSPlatform(OSPlatform.Linux);
		var inputDevice = isLinux ? "SharpSticks.LinuxInput.LinuxInputJoystickDevice" : "SharpSticks.DirectInput.DirectInputJoystickDevice";
		var inputFactory = isLinux ? "SharpSticks.LinuxInput.LinuxInputJoystickDeviceFactory" : "SharpSticks.DirectInput.DirectInputJoystickDeviceFactory";
		var outputDevice = isLinux ? "SharpSticks.LinuxOutput.LinuxOutputDevice" : "SharpSticks.VJoy.VJoyDevice";
		var outputFactory = isLinux ? "SharpSticks.LinuxOutput.LinuxOutputDeviceFactory" : "SharpSticks.VJoy.VJoyDeviceFactory";
		var combinedFactory = isLinux ? "SharpSticks.PlatformDefaults.LinuxDefaultDeviceFactory" : "SharpSticks.PlatformDefaults.WindowsDefaultDeviceFactory";

		if (HasType("SharpSticks.Console.ConsoleExtensions`2") && HasType(inputDevice) && HasType(outputDevice))
		{
			builder.AppendLine("global using static SharpSticks.Console.ConsoleExtensions;");
			builder.Append("global using static SharpSticks.Console.ConsoleExtensions<")
				.Append(inputDevice).Append(", ").Append(outputDevice).AppendLine(">;");
			wrote = true;
		}

		wrote |= AppendAlias(builder, "PlatformDefaultInputDevice", inputDevice, HasType);
		wrote |= AppendAlias(builder, "PlatformDefaultInputDeviceFactory", inputFactory, HasType);
		wrote |= AppendAlias(builder, "PlatformDefaultOutputDevice", outputDevice, HasType);
		wrote |= AppendAlias(builder, "PlatformDefaultOutputDeviceFactory", outputFactory, HasType);
		wrote |= AppendAlias(builder, "PlatformDefaultDeviceFactory", combinedFactory, HasType);

		return wrote ? builder.ToString() : null;
	}

	private static bool AppendAlias(StringBuilder builder, string alias, string typeName, Func<string, bool> hasType)
	{
		if (!hasType(typeName))
		{
			return false;
		...
		builder.Append("global using ").Append(alias).Append(" = ").Append(typeName).AppendLine(";");
		return true;
	}
	...
	private static void GenerateDeviceInfos(SourceProductionContext context, ImmutableArray<DeviceInfoTarget> targets)
	{
		...
		// Carry [OutputDevice] declarations to runtime via a module initializer, so the runtime can
		// materialize each device with its full declared capability (see DeclaredOutputDevices).
		EmitOutputDeviceRegistrations(context, coalesced);
	...
	// Emits a module initializer that registers every [OutputDevice] declaration into the runtime
	// registry as the consumer assembly loads. Deduped by device id (repeat declarations of one id
	// are equivalent). Emits nothing when no output devices are declared.
	private static void EmitOutputDeviceRegistrations(
		SourceProductionContext context, ImmutableArray<DeviceInfoTarget> targets)
	{
		using var byDeviceId = new PooledDictionary<uint, OutputDeviceDeclaration>();
		foreach (var target in targets)
		{
			if (target.OutputDevices.IsDefaultOrEmpty)
			{
				continue;
			}

			foreach (var declaration in target.OutputDevices)
			{
				byDeviceId[declaration.DeviceId] = declaration;
			}
		}

		if (byDeviceId.Count == 0)
		{
			return;
		}
		...
		builder.AppendLine();
		builder.AppendLine("internal static class __SharpSticksOutputDeviceRegistration");
		builder.AppendLine("{");
		builder.AppendLine("\t[ModuleInitializer]");
		builder.AppendLine("\tinternal static void Register()");
		builder.AppendLine("\t{");
		foreach (var declaration in byDeviceId.Values)
		{
			builder.Append("\t\tglobal::SharpSticks.OutputAbstractions.DeclaredOutputDevices.Register(")
				.Append(declaration.DeviceId).Append("u, ")
				.Append(declaration.ButtonCount).Append("u, ");
			if (declaration.Axes.IsDefaultOrEmpty)
			{
				builder.Append("ImmutableArray<global::SharpSticks.InputAbstractions.Axis>.Empty");
			}
			else
			{
				builder.Append("ImmutableArray.Create(");
				for (var index = 0; index < declaration.Axes.Length; index++)
				{
					if (index > 0)
					{
						builder.Append(", ");
					...
					builder.Append("global::SharpSticks.InputAbstractions.Axis.").Append(declaration.Axes[index].ToStringFast());
				}
		...
		context.AddSource("SharpSticksOutputDeviceRegistration.g.cs", SourceText.From(builder.ToString(), Encoding.UTF8));
	}
		...
		int indentLevel)
	{
		...
		if (HasLevel(levels, GenerateDeviceInfosLevels.DeviceNames))
		{
			...
			// Output devices with no input-side counterpart in the enumeration (the loopback isn't
			// live, or the platform doesn't surface one) still need name constants so the typed
			// output class and RenameAxis/RenameButton can reference them. When the loopback IS
			...
			foreach (var outputDevice in outputDevices.OrderBy(static device => device.DeviceId))
			{
				...
				var deviceName = string.IsNullOrEmpty(outputDevice.ProductName) ? baseName : outputDevice.ProductName;
				builder.Append(memberIndent)
					.Append("public const string ")
					.Append(baseName)
					.Append(" = ")
					.Append(SymbolDisplay.FormatLiteral(deviceName, quote: true))
					.AppendLine(";");
	...
	private static string GetOutputBaseName(OutputDeviceSnapshot device)
	{
		...
		return $"VJoyDevice{device.DeviceId}";
	}
			...
			ImmutableArray<DeviceRename> deviceRenames)
	{
		...
		return (merged, renames.ToImmutable());
	}
